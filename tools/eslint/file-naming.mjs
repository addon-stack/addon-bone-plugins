        ...
        if (declaration.type === "VariableDeclaration") {
            for (const variable of declaration.declarations) {
                if (variable.id.type === "Identifier" && variable.init?.type === "ClassExpression") {
                    bindings.set(variable.id.name, {
                        name: variable.init.id?.name ?? variable.id.name,
...
export default {
    meta: {name: "addon-bone-plugins-project-rules"},
    processors: {
        ...
        "file-naming": {
            meta: {
                ...
                messages: {
                    ...
                    className: "A file exporting class '{{className}}' must be named " +
                        "'{{className}}.{{extension}}' (PascalCase).",
                    ...
                    testName: "Test filenames must use kebab-case, or PascalCase for a class under test; " +
                        "keep suffixes such as .integration.test lowercase.",
                },
            },
            create(context) {
                return {
                    Program(program) {
                        ...
                        if (classes.includes(null)) {
                            ...
                            const invalidClassFilename = stem !== className
                                || !pascalCase.test(className)
                                || suffixes.length && !declarationSuffix;

                            if (invalidClassFilename) {
                                report("className", {
                                    className,
                                    extension: declarationSuffix ? `d.${extension}` : extension,
                                });
                            }
                        } else if (suffixes.includes("test") || suffixes.includes("spec")) {
                            const invalidTestFilename = !kebabCase.test(stem) && !pascalCase.test(stem)
                                || suffixes.some(suffix => !kebabCase.test(suffix));

                            if (invalidTestFilename) {
                                report("testName");
                            }
