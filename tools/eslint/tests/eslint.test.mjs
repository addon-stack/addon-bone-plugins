...
describe("ESLint project configuration", () => {
    test("checks formatting without editing and fixes blank lines and whitespace idempotently", async () => {
        ...
        expect(
            checked.messages.some(message => message.ruleId === "@stylistic/padding-line-between-statements")
        ).toBe(true);
    ...
    test("rejects source lines longer than 120 characters", async () => {
        const source = 'export const value = "' + "x".repeat(130) + '";\n';
        const [result] = await checker.lintText(source, {filePath: "src/long-line.ts"});

        expect(result.messages.some(message => message.ruleId === "@stylistic/max-len")).toBe(true);
    });

    test("requires braces around control flow bodies", async () => {
        const source = "export function check(value: boolean) {\nif (!value) return false;\nreturn true;\n}\n";
        const [checked] = await checker.lintText(source, {filePath: "src/check.ts"});

        expect(checked.messages.some(message => message.ruleId === "curly")).toBe(true);

        const [fixed] = await fixer.lintText(source, {filePath: "src/check.ts"});

        expect(fixed.messages).toEqual([]);

        expect(fixed.output).toBe([
            "export function check(value: boolean) {",
            "    if (!value) {",
            "        return false;",
            "    }",
            "",
    ...
    test("groups imports by platform, framework, packages, local code, assets and styles", async () => {
        const source = [
            'import "./theme.scss";',
            'import _iconUrl from "./icon.svg";',
            'import {local as _local} from "./local";',
            'import {globToRegex as _globToRegex} from "webext-patterns";',
            'import {getManifest as _getManifest} from "@addon-core/browser";',
            'import _registerContentScript from "@adnbn/plugin-reg-cs";',
            'import type {Config as _Config} from "adnbn/config";',
            'import {defineConfig as _defineConfig} from "adnbn";',
            'import _path from "node:path";',
            'import "./reset.css";',
            "",
        ].join("\n");

        const [result] = await fixer.lintText(source, {filePath: "src/import-groups.ts"});
    ...
    ])("ignores generated/dependency file %s", async filePath => {
    ...
    test.each([
        "src/module-name.ts",
        "src/api.d.ts",
        "src/module-name.test.ts",
        "src/Example.test.ts",
        "src/Example.integration.test.ts",
        "tests/module-name.spec.mjs",
        "tsup.config.ts",
    ])("accepts ordinary/test filename %s", async filePath => {
    ...
    test("rejects anonymous, multiple and non-PascalCase exported classes", async () => {
        ...
        expect(
            (await messagesFor("export class One {} export class Two {}", "src/One.ts"))[0].messageId
        ).toBe("multiple");
    ...
    ])("rejects filename %s without renaming it", async filePath => {
    ...
    test.each([
        ...
        ".husky/pre-commit",
        "package.json",
    ])("preserves standard filename %s", async filePath => {
