...
const lintStagedConfig = (await import(new URL("../../../.lintstagedrc.mjs", import.meta.url))).default;
...
test("pre-commit fixes staged files before testing without formatting the whole worktree", () => {
    const commands = readFileSync(join(root, ".husky/pre-commit"), "utf8")
        .split("\n")
        .filter(line => line.startsWith("pnpm "));

    expect(commands).toEqual(["pnpm exec lint-staged"]);
    expect(lintStagedConfig).toEqual({"*": "eslint --fix --max-warnings 0 --no-warn-ignored --"});
});
...
describe("pre-commit staged formatting in an isolated Git clone", () => {
    ...
    const lintStaged = () => run(process.execPath, [
        join(root, "node_modules/lint-staged/bin/lint-staged.js"),
        "--quiet",
    ]);

    beforeEach(() => {
        temporaryDirectory = mkdtempSync(join(tmpdir(), "addon-bone-plugins-pre-commit-"));
        ...
        for (const file of ["package.json", ".lintstagedrc.mjs", "eslint.config.mjs", "tools/eslint"]) {
    ...
    test("allows ignored lockfiles and staged deletions without ignored-file warnings", () => {
        ...
        write(".npmrc", `${source}\n`);
        git("add", "--", ".npmrc");
        git("rm", "--", "packages/@adnbn/plugin-reg-cs/plugin/index.ts");

        const result = lintStaged();
        expect(result.stderr).toBe("");
        expect(result.status).toBe(0);
        expect(staged(".npmrc")).toBe(`${source}\n`);
        expect(git("diff", "--cached", "--name-status")).toContain("D\tpackages/@adnbn/plugin-reg-cs/plugin/index.ts");
    });
